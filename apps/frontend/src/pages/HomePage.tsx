...
export function HomePage() {
  ...
  const { data: trending, isLoading: trendingLoading } = useTrending(trendingWindow);
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-12 flex flex-col gap-14">
        <section aria-labelledby="trending-heading">
          <SectionHeader title="Trending" id="trending-heading">
            <TrendingToggle value={trendingWindow} onChange={setTrendingWindow} />
          </SectionHeader>
          <MovieRow movies={trendingMovies} isLoading={trendingLoading} />
        </section>

        <section aria-labelledby="now-playing-heading">
          <SectionHeader title="Now Playing" id="now-playing-heading" />
