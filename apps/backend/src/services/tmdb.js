...
// Conservative HTTPS agent: no keep-alive to avoid connection state issues
...
tmdbClient.interceptors.response.use(
    ...
    async (err) => {
        ...
        if (isNetworkError && (config._retries ?? 0) < MAX_RETRIES) {
            ...
            // Conservative exponential backoff: 1s, 2s, 4s, 8s, 16s
            const delay = RETRY_DELAY_MS * Math.pow(2, config._retries - 1);
            console.warn(
                `[TMDB] ${err.code} — retry ${config._retries}/${MAX_RETRIES} in ${delay}ms`,
            );
